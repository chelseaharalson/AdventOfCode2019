...
public class Day11 {
    ...
    public static void main(String[] args) {
        ...
        while (computer.isRunning()) {
            List<Long> outputs = computer.getOutputList();
            // Output Position: what color and where to go next
            for (; outputPosition < outputs.size(); outputPosition += 2) {
                ...
                // Turn left from where facing
                if (turn == 0) {
                    directions--;
                } 
                // Turn right from where facing
                else {
                    directions++;
                }
                // Valid directions: 0, 1, 2, 3. If -1, make it 3. If 4, make it 0.
                if (directions == -1) {
                    directions += 4;
                }
                if (directions == 4) {
        ...
        // PART A
        System.out.println("Amount of panels: " + positions.size());
        
        // PART B
        // Get boundaries of where robot has painted
        int minX = 10000;
        int minY = 10000;
        int maxX = -10000;
        int maxY = -10000;
